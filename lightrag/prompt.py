...
PROMPTS["entity_extraction"] = """---Goal---
Given a text chunk from a medical guide and the medical guide title, extract entities and relationships for medical guide retrieval:

---Entity Types---
1. "primary_subject": The main topic of the medical guide (extracted once from the guide title)
2. "document_section": Specific headings/sections mentioned in the text chunk

---Steps---
1. Extract PRIMARY_SUBJECT entity:
   - Extract ONE "primary_subject" entity from the medical guide title: {title}
   - Use proper medical terminology and capitalization
   - Format: ("entity"{tuple_delimiter}<primary_subject_name>{tuple_delimiter}"primary_subject"{tuple_delimiter}<description>)

2. Extract DOCUMENT_SECTION entities:
   - Identify all section headings, subsections, or clinical topics mentioned in the text chunk
   - Each should represent a specific part of medical guidelines (e.g., "Risk Factors", "Management", "Diagnosis")
   - Format: ("entity"{tuple_delimiter}<section_name>{tuple_delimiter}"document_section"{tuple_delimiter}<description>)

3. Build relationships:
   - Create relationships between the primary_subject and each document_section
   - Each relationship indicates that the section belongs to or is part of the main medical topic
   - Format: ("relationship"{tuple_delimiter}<primary_subject>{tuple_delimiter}<document_section>{tuple_delimiter}<relationship_description>{tuple_delimiter}"medical_guide_structure"{tuple_delimiter}<strength>)
...
Output:
("entity"{tuple_delimiter}"Primary Open-Angle Glaucoma"{tuple_delimiter}"primary_subject"{tuple_delimiter}"Primary Open-Angle Glaucoma is the main medical condition covered in this preferred practice pattern guide."){record_delimiter}
("entity"{tuple_delimiter}"Risk Factors"{tuple_delimiter}"document_section"{tuple_delimiter}"Risk Factors section discusses factors that increase the likelihood of developing primary open-angle glaucoma including age, intraocular pressure, and family history."){record_delimiter}
("entity"{tuple_delimiter}"Diagnosis"{tuple_delimiter}"document_section"{tuple_delimiter}"Diagnosis section covers the comprehensive eye examination procedures including gonioscopy and visual field testing for glaucoma detection."){record_delimiter}
("entity"{tuple_delimiter}"Management"{tuple_delimiter}"document_section"{tuple_delimiter}"Management section outlines treatment strategies including medical therapy and surgical interventions for glaucoma care."){record_delimiter}
("relationship"{tuple_delimiter}"Primary Open-Angle Glaucoma"{tuple_delimiter}"Risk Factors"{tuple_delimiter}"Risk Factors section is part of the Primary Open-Angle Glaucoma practice pattern guide."{tuple_delimiter}"medical_guide_structure"{tuple_delimiter}10){record_delimiter}
("relationship"{tuple_delimiter}"Primary Open-Angle Glaucoma"{tuple_delimiter}"Diagnosis"{tuple_delimiter}"Diagnosis section is part of the Primary Open-Angle Glaucoma practice pattern guide."{tuple_delimiter}"medical_guide_structure"{tuple_delimiter}10){record_delimiter}
("relationship"{tuple_delimiter}"Primary Open-Angle Glaucoma"{tuple_delimiter}"Management"{tuple_delimiter}"Management section is part of the Primary Open-Angle Glaucoma practice pattern guide."{tuple_delimiter}"medical_guide_structure"{tuple_delimiter}10){completion_delimiter}
#############################""",
...
Output:
("entity"{tuple_delimiter}"Age-Related Macular Degeneration"{tuple_delimiter}"primary_subject"{tuple_delimiter}"Age-Related Macular Degeneration is the main medical condition addressed in this preferred practice pattern guide."){record_delimiter}
("entity"{tuple_delimiter}"Epidemiology"{tuple_delimiter}"document_section"{tuple_delimiter}"Epidemiology section presents data on the prevalence and incidence patterns of age-related macular degeneration."){record_delimiter}
("entity"{tuple_delimiter}"Prevention"{tuple_delimiter}"document_section"{tuple_delimiter}"Prevention section outlines strategies for preventing AMD including lifestyle modifications and nutritional supplements."){record_delimiter}
("entity"{tuple_delimiter}"Treatment Recommendations"{tuple_delimiter}"document_section"{tuple_delimiter}"Treatment Recommendations section provides guidance on managing different types of AMD including anti-VEGF therapy for wet AMD."){record_delimiter}
("relationship"{tuple_delimiter}"Age-Related Macular Degeneration"{tuple_delimiter}"Epidemiology"{tuple_delimiter}"Epidemiology section is part of the Age-Related Macular Degeneration practice pattern guide."{tuple_delimiter}"medical_guide_structure"{tuple_delimiter}10){record_delimiter}
("relationship"{tuple_delimiter}"Age-Related Macular Degeneration"{tuple_delimiter}"Prevention"{tuple_delimiter}"Prevention section is part of the Age-Related Macular Degeneration practice pattern guide."{tuple_delimiter}"medical_guide_structure"{tuple_delimiter}10){record_delimiter}
("relationship"{tuple_delimiter}"Age-Related Macular Degeneration"{tuple_delimiter}"Treatment Recommendations"{tuple_delimiter}"Treatment Recommendations section is part of the Age-Related Macular Degeneration practice pattern guide."{tuple_delimiter}"medical_guide_structure"{tuple_delimiter}10){completion_delimiter}
#############################""",
...
Output:
("entity"{tuple_delimiter}"Cataract in the Adult Eye"{tuple_delimiter}"primary_subject"{tuple_delimiter}"Cataract in the Adult Eye is the main medical condition covered in this preferred practice pattern guide."){record_delimiter}
("entity"{tuple_delimiter}"Preoperative Evaluation"{tuple_delimiter}"document_section"{tuple_delimiter}"Preoperative Evaluation section covers assessment of visual symptoms and comprehensive eye examination before cataract surgery."){record_delimiter}
("entity"{tuple_delimiter}"Surgical Techniques"{tuple_delimiter}"document_section"{tuple_delimiter}"Surgical Techniques section details phacoemulsification procedures and intraocular lens selection for cataract surgery."){record_delimiter}
("entity"{tuple_delimiter}"Postoperative Care"{tuple_delimiter}"document_section"{tuple_delimiter}"Postoperative Care section addresses monitoring for complications and visual rehabilitation after cataract surgery."){record_delimiter}
("relationship"{tuple_delimiter}"Cataract in the Adult Eye"{tuple_delimiter}"Preoperative Evaluation"{tuple_delimiter}"Preoperative Evaluation section is part of the Cataract in the Adult Eye practice pattern guide."{tuple_delimiter}"medical_guide_structure"{tuple_delimiter}10){record_delimiter}
("relationship"{tuple_delimiter}"Cataract in the Adult Eye"{tuple_delimiter}"Surgical Techniques"{tuple_delimiter}"Surgical Techniques section is part of the Cataract in the Adult Eye practice pattern guide."{tuple_delimiter}"medical_guide_structure"{tuple_delimiter}10){record_delimiter}
("relationship"{tuple_delimiter}"Cataract in the Adult Eye"{tuple_delimiter}"Postoperative Care"{tuple_delimiter}"Postoperative Care section is part of the Cataract in the Adult Eye practice pattern guide."{tuple_delimiter}"medical_guide_structure"{tuple_delimiter}10){completion_delimiter}
#############################"""
]

# 这个修改后的提示词具有以下特点：
#
# 1. **明确的两层结构**：只有 `primary_subject` 和 `document_section` 两种实体类型
# 2. **单一主题提取**：从医疗指南标题中只提取一次 `primary_subject`
# 3. **章节重点提取**：从文本块中提取所有相关的 `document_section`
# 4. **结构化关系**：每个章节都与主题建立关系，便于检索时理解指南结构
# 5. **医疗指南特化**：示例涵盖了常见的眼科医疗指南结构
